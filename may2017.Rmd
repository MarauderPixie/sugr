---
title: "Glucose Diary of May 2017"
output: 
  html_document
---
...
## read data
sugr    <- readRDS("./data/all_sugr.rds") %>% filter(Monat == "May", Jahr == "2017")
base    <- readRDS("./data/base.rds")
bexpert <- readRDS("./data/all_bexpert.rds") %>% filter(Monat == "May", Jahr == "2017")
...
**05.06.2017:**  
I haven't thought about this for quite some time, but today I did. Turns out: the difference in means is quite easy to explain: When all (or at least most) high measurements get handed on to the _BolusExpert_, but many low and okay measurements don't, the mean rises. So Captain Obvious strikes again!

### Old Ponderings

Now this is really something I'm scratching my head over: the pump seems to have two ways to measure glucose levels (or at least there are two columns with different timestamps and values). The first is a somewhat _"regular"_ column internally called ``r sjmisc::get_label(sugr$BZ_Wert)[[1]]`` and another one called ``r sjmisc::get_label(sugr$BE_BZ)[[1]]`` (translated _"GL-Measurement"_ and _"BolusExpert: GL-Input"_ respectively).  
...
What is still bothering me is the following:  
